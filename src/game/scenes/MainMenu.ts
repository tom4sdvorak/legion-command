...
export class MainMenu extends Scene
{
    ...
    create ()
    {
        ...
        this.mainMenu = new UIComponent(this, (this.game.config.width as number)/4, (this.game.config.height as number)/2, (this.game.config.width as number)*0.3, (this.game.config.height as number)*0.9, 0);
        this.add.existing(this.mainMenu);


        this.logo = this.add.text(0, 0, 'Game', {
            fontFamily: 'Arial Black', fontSize: 64, color: '#ffffff',
            stroke: '#000000', strokeThickness: 8,
            align: 'center'
        }).setOrigin(0.5);
        this.mainMenu.add(this.logo);
